...
def interviewee():
    if request.method == 'POST' and 'username' in request.form and 'usermail' in request.form and\
            'userpassword' in request.form:
...
def predict():
    ...
    if request.method == 'POST':
        ...
        result = {'Name': fname+' '+lname, 'Age': age, 'Email': email, 'Mobile Number': data.get('mobile_number', None),
                  ...
                  'Degree': data.get('degree', None)[0], 'Designation': data.get('designation', None)[0],
...
def accept():
    ...
    msg.body = f"Dear {name},\n\n" + f"Thank you for taking the time to interview for the {position} position." \
                                     f"We enjoyed getting to know you. We have completed all of our interviews.\n\n" +\
               f"I am pleased to inform you that we would like to offer you the {position} position." \
               f"We believe your past experience and strong technical skills will be an asset to our organization." \
               f"Your starting salary will be $15,000 per year with an anticipated start date of July 1.\n\n" +\
...
def reject():
    ...
    msg.body = f"Dear {name},\n\n" + f"Thank you for taking the time to consider Smart Hire." \
                                     f"We wanted to let you know that we have chosen to move forward with" \
                                     f"a different candidate for the {position} position.\n\n" +\
               f"Our team was impressed by your skills and accomplishments." \
               f"We think you could be a good fit for other future openings and will reach out again" \
               f"if we find a good match.\n\n" +\
               f"We wish you all the best in your job search and future professional endeavors.\n\n" +\
               f"Regards,\n\n" + f"Harsh Verma\nHuman Resources Director\n " \
                                 f"Phone: 555-555-1234\nEmail: feedbackmonitor123@gmail.com"
    mail.send(msg)
